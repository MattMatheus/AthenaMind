...
- Updated operator workflow startup routine to include Go preflight before cycle execution:
  - `OPERATOR_DAILY_WORKFLOW.md`
- Updated how-to index to include the new setup guide:
  - `docs/how-to/README.md`
- Minor consistency fix to satisfy existing doc gate:
  - `research/handoff.md` now references `research/roadmap/PHASED_IMPLEMENTATION_PLAN_V01_V03.md`.
...
## Test Updates Made
- Added `scripts/test_go_toolchain_readiness.sh` to enforce:
  - preflight script existence,
  - setup doc existence,
  - setup doc references to `go.mod`, `scripts/check_go_toolchain.sh`, and `go test ./...`,
  - setup doc minimum version matches parsed `go.mod` version,
  - operator workflow includes preflight command.

## Test Run Results
- `./scripts/check_go_toolchain.sh` -> PASS
