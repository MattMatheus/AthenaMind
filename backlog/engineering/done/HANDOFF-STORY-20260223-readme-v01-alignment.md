...
- Added explicit roadmap pointer to `research/roadmap/PHASED_IMPLEMENTATION_PLAN_V01_V03.md`.
- Added preserved long-term vision doc at `docs/product/VISION.md`.
- Updated `docs/product/README.md` to include the new vision artifact.
- Added docs regression test `scripts/test_readme_v01_alignment.sh` and wired it into `scripts/run_doc_tests.sh`.
- Updated `research/roadmap/PROGRAM_STATE_BOARD.md` queue counts to match filesystem truth during this cycle’s test gate run.
...
## Test Updates Made
- New: `scripts/test_readme_v01_alignment.sh`
...
- Updated runner: `scripts/run_doc_tests.sh` includes the new test script.
...
- `docs/product/VISION.md` is preserved from current product-vision sources and summarized for long-term direction; if exact historical README wording is required verbatim, a follow-up archival artifact may be needed.
- `scripts/run_doc_tests.sh` still emits non-blocking warning during launcher tests when `memory-cli` is not installed on PATH (`memory bootstrap skipped; 'memory-cli' not found`).
...
- Verify README acceptance criteria directly:
  ...
  - vision content preserved in `docs/product/VISION.md`
