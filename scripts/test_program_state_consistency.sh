...
program_board="$root_dir/research/roadmap/PROGRAM_STATE_BOARD.md"
readiness="$root_dir/research/roadmap/CODING_READINESS_DECISION_2026-02-22.md"
plan_session="$root_dir/research/planning/sessions/PLAN-20260222-idea-generation-session.md"
research_backlog="$root_dir/research/roadmap/RESEARCH_BACKLOG.md"
...
actual_eng_intake="$(find "$root_dir/backlog/engineering/intake" -maxdepth 1 -type f -name '*.md' ! -name '*TEMPLATE*' | wc -l | tr -d ' ')"
actual_eng_active="$(find "$root_dir/backlog/engineering/active" -maxdepth 1 -type f -name '*.md' ! -name 'README.md' | wc -l | tr -d ' ')"
actual_eng_qa="$(find "$root_dir/backlog/engineering/qa" -maxdepth 1 -type f -name '*.md' | wc -l | tr -d ' ')"
actual_eng_done="$(find "$root_dir/backlog/engineering/done" -maxdepth 1 -type f -name 'STORY-*.md' | wc -l | tr -d ' ')"
actual_arch_intake="$(find "$root_dir/backlog/architecture/intake" -maxdepth 1 -type f -name '*.md' ! -name '*TEMPLATE*' | wc -l | tr -d ' ')"
actual_arch_active="$(find "$root_dir/backlog/architecture/active" -maxdepth 1 -type f -name '*.md' ! -name 'README.md' | wc -l | tr -d ' ')"
actual_arch_qa="$(find "$root_dir/backlog/architecture/qa" -maxdepth 1 -type f -name '*.md' | wc -l | tr -d ' ')"
actual_arch_done="$(find "$root_dir/backlog/architecture/done" -maxdepth 1 -type f -name 'ARCH-*.md' | wc -l | tr -d ' ')"
...
if rg -q '^-[[:space:]]*`GO`' "$readiness"; then
  if rg -q 'backlog/engineering/active/' "$readiness"; then
...
if rg -q '^-[[:space:]]*`status`:[[:space:]]*draft' "$plan_session"; then
  ...
  while IFS= read -r line; do
    ...
    base="$(basename "$path")"
    if [[ -f "$root_dir/backlog/engineering/done/$base" || -f "$root_dir/backlog/architecture/done/$base" ]]; then
      has_downstream_done=1
      break
    fi
  done < <(rg -n 'backlog/(engineering|architecture)/(intake|active)/[^`]+\.md' "$plan_session" | sed 's/^[0-9]*://')
