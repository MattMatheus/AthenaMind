...
Operating mode: humans manage/approve direction; implementation is agent-owned.
...
- Default execution branch: `dev`.
- Optional isolated branch for parallel agent work: `agent/<agent-id>/<cycle-id>`.
- Release checkpoint IDs are date-based: `<label>-YYYY-MM-DD`.
- Merge policy:
  - `dev -> main`: requires human approval.
  - `dev -> dev` and `dev -> feature/*`: may be automated by agents.
- Human role in this mode:
  - approve priorities, direction changes, and release promotions
  - do not perform direct code implementation
- Full policy:
  - `operating-system/playbooks/agent-branch-strategy.md`

## Security Launch Control
- No `dev -> prod` transition without operator validation.
- For security-backing contract changes, launch requires:
  - one-time nonce
  - primary OTP
  - security OTP
  - explicit authorization phrase: `LAUNCH AUTHORIZED`
- Nonce window is tunable via `ATHENA_SECURITY_GATE_WINDOW_MINUTES` (default `15`).
- Generate nonce with:
  - `./tools/generate_security_nonce.sh`
- CI security gate:
  - `./tools/check_security_nonce_gate.sh`
- Protected contract list:
  - `operating-system/contracts/SECURITY_BACKING_CONTRACT_PATHS.txt`
  - includes `AGENTS.md` as a protected security document.
...
- `done` is not automatically `shipped`:
  ...
  - use date-based release IDs: `<label>-YYYY-MM-DD`.
...
- Ingest artifact bundle (V1 -> V2): `./tools/ingest_artifact_bundle.sh --zip <bundle.zip>`
- Generate launch authorization package: `./tools/generate_launch_authorization_package.sh`
- Validate launch authorization package: `./tools/validate_launch_authorization_package.sh --package <path>`
