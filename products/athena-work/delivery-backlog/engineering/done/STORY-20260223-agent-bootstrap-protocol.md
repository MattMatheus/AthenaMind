...
## Problem Statement
- The memory CLI exists but nothing consumes it. The cross-pillar bootstrap order (load procedural → retrieve semantic → execute → persist) is documented in `product-research/architecture/FOUNDATION_PILLARS.md` but not implemented. Without a concrete bootstrap protocol, the memory system cannot prove its core value proposition: instant resume and cross-session continuity.
