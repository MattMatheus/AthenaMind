...
- `source_story`: delivery-backlog/active/STORY-20260222-doc-test-harness-standardization.md
- `status`: done
...
## Summary
`tools/launch_stage.sh engineering` uses a sed expression with `\+` that does not parse as expected on BSD sed, so ordered entries in `delivery-backlog/active/README.md` are not read. The launcher silently falls back to alphabetical file ordering, which can execute stories out of ranked sequence.

## Expected Behavior
- Top story should be selected from `delivery-backlog/active/README.md` sequence when present and valid.

## Actual Behavior
- README parsing returns empty and launcher selects first filename alphabetically.

## Reproduction Steps
1. Ensure `delivery-backlog/active/README.md` lists a top story not alphabetically first.
