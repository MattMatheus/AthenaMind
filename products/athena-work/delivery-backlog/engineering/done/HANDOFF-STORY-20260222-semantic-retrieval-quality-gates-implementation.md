...
## What Changed
- Added retrieval quality evaluation harness command `evaluate` in `<repo>/cmd/memory-cli/main.go`.
...
- Added pinned harness inputs at `<repo>/cmd/memory-cli/testdata/eval-query-set-v1.json` with fixed corpus/query/config ids in the CLI.
- Expanded tests in `<repo>/cmd/memory-cli/main_test.go` for fallback determinism, response metadata completeness, and evaluation metric behavior.
...
## Test Updates Made
- Added evaluation and determinism-focused tests in `<repo>/cmd/memory-cli/main_test.go`.
