...
## Scope
- In:
  - Modify `tools/launch_stage.sh` to invoke `memory-cli bootstrap` before emitting the seed prompt
  - Append bootstrap payload to the seed prompt context (or output it alongside the prompt)
  - Modify `tools/run_observer_cycle.sh` to invoke `memory-cli episode write` with cycle outcome data after observer report generation
