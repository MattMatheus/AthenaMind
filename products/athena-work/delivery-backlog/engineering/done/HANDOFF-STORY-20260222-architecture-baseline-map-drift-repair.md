...
- Replaced stale architecture intake links with current queue-state references:
  - architecture queue empty state (`delivery-backlog/architecture/active/README.md`)
  - architecture completion artifacts in `delivery-backlog/architecture/done/`
  - implementation-side open work in `delivery-backlog/engineering/active/`
- Moved story from `delivery-backlog/engineering/active/` to `delivery-backlog/engineering/qa/` and updated status to `qa`.
...
- AC1 (no missing intake refs): PASS
  - Removed references to missing `delivery-backlog/architecture/intake/ARCH-20260222-*.md` files.
...
- AC3 (doc tests green): PASS
  - `tools/run_doc_tests.sh` completed successfully.
...
- Verify no stale references remain under `delivery-backlog/architecture/intake/` except template.
