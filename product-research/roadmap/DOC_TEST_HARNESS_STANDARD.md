...
## Pattern
1. Add a story-specific test script at `tools/test_<story-scope>.sh`.
2. Source the shared harness helper from `tools/lib/doc_test_harness.sh`.
3. Use shared assertions:
   - `doc_assert_exists`
   - `doc_assert_contains`
4. End tests with `doc_test_finish`.
5. Add new story-specific test script to `tools/run_doc_tests.sh`.
